...
lazy_static! {
    pub static ref XKB_TO_PHYSICAL_KEY_MAP: HashMap<u64, u64> =  HashMap::from([
    ...
    pub static ref GTK_KEYVAL_TO_LOGICAL_KEY_MAP: HashMap<u64, u64> =  HashMap::from([
...
/* // Original C++ function: initialize_modifier_bit_to_checked_keys
fn initialize_modifier_bit_to_checked_keys(table: &mut HashMap<u32, FlKeyEmbedderCheckedKey>) {
    let data = FlKeyEmbedderCheckedKey {
        is_caps_lock: false,
    ...
    table.insert(GDK_SHIFT_MASK, data);

    // Repeat similar steps for other modifier keys
}

// Original C++ function: initialize_lock_bit_to_checked_keys
fn initialize_lock_bit_to_checked_keys(table: &mut HashMap<u32, FlKeyEmbedderCheckedKey>) {
    let data = FlKeyEmbedderCheckedKey {
        is_caps_lock: true,
        primary_physical_key: 0x000070039,  // capsLock
        primary_logical_key: 0x00100000104, // capsLock
    };
    table.insert(GDK_LOCK_MASK, data);

    // Repeat similar steps for other lock keys
}

const KVALUE_MASK: u64 = 0x000ffffffff;
const UNICODE_PLANE: u64 = 0x00000000000;
const GTK_PLANE: u64 = 0x01500000000; */
